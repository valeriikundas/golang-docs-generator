...
from utils import (get_lines_above_that_start_with_comment_sign,
    ...
    def write(self, doc_file):
        with open(doc_file, "w") as doc_file:
            ...
            for class_name, class_methods in self.class_methods.items():
                methods_content.append(f"<h4>{class_name}</h4>")
...
def generate_content_files(curpath, source_code_structure, docs_path, code_path):
    content = ""
    for name, path in source_code_structure.items():
        if type(path) is str:
            ...
            generate_content_files(
                os.path.join(curpath, name), path, docs_path, code_path
            )
...
def build_doc_files(source_code_structure_or_filename_bad_name_for_variable, doc_path):
    ...
    for dirname, nested in list(
        ...
        build_doc_files(nested, os.path.join(doc_path, dirname))
